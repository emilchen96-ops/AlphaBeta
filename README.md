...
> **UX03-R 研究体验收口：** 主菜单已统一为“策略回测、AI调研、研究档案”。
> `/research/archive` 按策略回测、智能选股和 AI 调研统一展示可读历史；批量独立回测
> 提供后端汇总总报告、分布图、失败原因、个股下钻和 CSV。单股回测仍默认使用下一
> 交易日开盘成交，也可显式选择使用 RAW 1 分钟行情的 14:55 当日尾盘判断模式。
> 旧资讯/市场事件入口会跳转到 AI 调研的“调研资料与来源”，后台证据事实继续保留。
> 详见 [UX03-R 使用与边界](docs/ux03_research_archive.md)。

> **CAL01-R 交易日历与选股窗口修复：** 默认产品模式为 `RESEARCH_ONLY`。智能选股现已包含自然语言、
> 系统模板、我的方案和历史结果，并可将真实结果加入自选或预填快速回测。推荐从
> `http://127.0.0.1:5173/scanners` 用自然语言或模板确认安全规则；开始后会自动检查
> 正式日线覆盖、只补 MiniQMT 真实开放日缺口、跳过法定休市和已知停牌日，并区分
> 新股历史不足、长期无行情、日历异常、Provider 失败与质量失败。快速回测仍不会向
> MiniQMT 或券商发送订单。详见
> [CAL01-R 日历与选股窗口语义](docs/cal01r_calendar_screening_semantics.md)。
...
启动服务并完成本地历史行情补数后，打开 `http://127.0.0.1:5173/strategy-research`。
默认模式下，T 日收盘信号最早在下一根可用日线开盘成交；显式选择“当日尾盘成交”时，
系统使用 14:55 前可见的分钟行情判断，并在下一根可用分钟 K 线尝试成交。也可在 API
容器或已激活的后端环境运行：
