...
class ScanRunMemberRepository(Protocol):
...
class MultiAgentResearchTaskRepository(Protocol):
...
class MultiAgentResearchStepRepository(Protocol):
...
class MultiAgentResearchReportRepository(Protocol):
...
class UserScreeningRepository(Protocol):
...
class BacktestBatchRepository(Protocol):
    async def lock_idempotency_key(self, key: str) -> None: ...
