...
def test_configuration_fingerprint_captures_data_source_and_actual_risk_limits() -> None:
    snapshot = BacktestRiskConfigurationSnapshot(
        reference="r01-settings-v1",
        limits_version_marker="r01-config-v2",
        max_order_notional=Decimal("100000"),
        max_instrument_weight=Decimal("0.5"),
    ...
    baseline = replace(
        _configuration(),
        risk_configuration_reference=snapshot.reference,
        risk_configuration_snapshot=snapshot,
        data_source_code="baostock",
    )
    changed_snapshot = replace(snapshot, max_order_notional=Decimal("200000"))
    changed_risk = replace(baseline, risk_configuration_snapshot=changed_snapshot)
    changed_source = replace(baseline, data_source_code="BT01_DEMO")

    assert baseline.data_source_code == "BAOSTOCK"
    assert backtest_request_fingerprint(baseline) != backtest_request_fingerprint(changed_risk)
    assert backtest_request_fingerprint(baseline) != backtest_request_fingerprint(changed_source)
    assert backtest_risk_configuration_marker(snapshot) != backtest_risk_configuration_marker(
        changed_snapshot
    )
    restored = backtest_configuration_from_dict(backtest_configuration_to_dict(baseline))
    assert restored.risk_configuration_snapshot == snapshot
    assert restored.data_source_code == "BAOSTOCK"
    assert backtest_request_fingerprint(restored) == backtest_request_fingerprint(baseline)
...
def test_profit_factor_is_null_when_there_are_no_losses() -> None:
    ...
    assert "PROFIT_FACTOR_UNDEFINED_NO_LOSS_TRADES" in metrics.warnings
