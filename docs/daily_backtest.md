...
- 默认 `NEXT_OPEN`：T 日收盘确认信号，下一交易日按 RAW 开盘价并计入滑点；
  BUY 默认最多接受相对信号日 RAW 收盘价 5% 的高开，可关闭或修改该限制。
- `DAY` 表示下一交易日未成交即取消，`GTC` 表示继续等待后续交易日；
  历史运行保留 `SIGNAL_CLOSE_LIMIT` 语义，不会被新规则改写。
- 固定费用、滑点和最大成交量参与率。
