...
## 当前增量：BT02-A 完成开发，等待在线环境验收

BT02-A 已在 BT01 事实链上实现日线策略的分钟级确认与执行、安全候选日预筛、MiniQMT 日线/
分钟缺口自动准备、单股/自选/全 A 股独立回测、后台取消/失败项重试、分钟审计与批量总报告。
确定性后端测试和受控性能基准已通过；Docker Migration、真实 MiniQMT 下载与浏览器端到端验收
受当前环境限制尚未完成。详见 [BT02-A](bt02_intraday_backtest.md) 和
[验收记录](verification/bt02a_acceptance.md)。不包含 Tick、Level-2、高频撮合或真实交易。

## SC02-D 完成开发

SC02-D 在 SC02-C 产品闭环上增加后台数据编排：按 ScreeningSpec 推导交易日窗口和
参考事实，只补 MiniQMT 缺失区间，复检质量，预热所需特征后调用既有规则引擎。页面
提供分阶段进度、取消、失败股票重试和“仅使用当前数据”高级选项。该阶段不包含 SC03
批量回测或研究报告。

SC02-C 收口了标准条件选股的产品闭环：模板、个人方案版本、历史结果、自选股和快速回测
预填。后续 SC03 可规划盘中增量筛选、调度和通知，但不得在未独立评审前接入实盘交易。

## UX02-B

研究工作台已收敛为快速回测、策略模板、参数对比和我的策略；StrategySpec、安全解析、
规则编辑、版本快照与 BT01 快速回测为同一产品流程。下一步仅规划真实使用反馈和小范围
UX 修复，不在本阶段开放交易或任意代码执行。

> 2026-07-23：MD01 已将 MiniQMT 确立为唯一正式行情源，合并“行情/实时行情”和“数据中心/
> 分钟数据”，并以来源过滤隔离历史兼容 Provider 与测试数据。MiniQMT 当前仅提供只读行情，
> 不包含账户、资金、持仓、订单或成交接入。详见 [MD01 行情使用说明](md01_market_data.md)。
...
> 分批查询端口、API、CLI 与页面。该段为 D03 封板时的历史状态；其后 MD01 已接入 MiniQMT，
> BT02-A 已完成开发，实盘仍未实现。
...
> 已在其后完成；该段为 D02 封板时的历史状态，后续 MD01 与 BT02-A 已完成开发，实盘仍未实现。
