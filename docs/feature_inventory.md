...
统计口径为下表 19 个用户可识别能力：`WORKING=3`、`NEEDS_DATA=10`、`NEEDS_CONFIG=2`、`PARTIAL=1`、`PLACEHOLDER=2`、`BROKEN=0`、`HIDDEN=0`、`NOT_IMPLEMENTED=1`。
...
| Backtests | `/backtest` | 只读查看状态 | 无 | 当前分支无 backtest API | 当前分支无可用服务 | 未纳入当前链 | 无 | PARTIAL | BT01 独立提交不能安全 cherry-pick，入口无启动按钮。 |
...
当前没有统一 Strategy CLI、BT01 CLI、Audit CLI、Settings CLI、Windows Agent 或 MiniQMT CLI。
