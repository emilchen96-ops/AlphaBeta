...
启动后打开 `http://127.0.0.1:5173/research/backtest`，输入
“10日价格突破 + 1.2倍成交量，5日均线退出，单只股票、两年日线”，解析并确认规则，
搜索股票后点击“开始回测”。MiniQMT 不登录时仍可查看已有结果；新回测要求对应历史日线
已经同步到本地数据库。
...
进入“自然语言选股”描述条件并确认预览，或从“选股模板”载入规则；执行完成后在真实结果中
批量加入自选，或者对单只股票打开快速回测。常用规则可保存到“我的选股方案”，所有运行可
在“历史结果”复查。再次运行和快速回测都需要用户最终确认。
