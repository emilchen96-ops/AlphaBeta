...
MiniQMT 只读行情的历史补数队列不是交易命令流，也不使用尚未启用的订单 Redis Streams。
它是单 Windows Agent 消费的受限工作队列，但仍遵守至少一次处理原则：读取队首不等于完成，
只有 API 已幂等持久化全部 K 线后才删除任务。Agent 崩溃、API 超时或电脑重启不会令已领取任务
消失。

- 等价的证券集合、周期和时间范围生成稳定请求指纹；指纹在任务完成或进入死信前持续有效，
  防止慢队列期间重复入队。
- 失败任务轮转到队尾并累计次数，避免一个异常证券造成队首阻塞；连续失败 3 次进入独立死信
  列表并释放去重标记。
- Agent 启动时压缩历史遗留的重复或损坏项，并优先处理历史任务；自动维护和实时快照异常不得
  饿死历史队列。
- PostgreSQL K 线唯一键仍是最终幂等边界，因此成功响应丢失造成的再次处理不会制造重复事实。

具体键名、启动方式和故障定位见 [MiniQMT 行情代理](miniqmt_market_data_agent.md)，架构决策见
[ADR 0020](adr/0020-miniqmt-history-recoverable-queue.md)。
