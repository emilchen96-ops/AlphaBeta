...
1. 在根目录 `.env` 配置模型服务并重启 API 与 AI 调研 Worker；Fake 只用于验收。
2. 确认数据中心已有目标股票所选范围内的 MiniQMT 本地日线。
3. 打开“AI 调研”，搜索股票，填写研究问题，选择深度和资料范围。
4. 点击“开始 AI 调研”。任务进入后台后可以关闭页面。
5. 在任务页观察角色进度；失败可查看稳定错误码并重新尝试，运行中可取消。
6. 完成后查看十章节报告，核对资料来源与局限，并可下载 Markdown/PDF。
7. 历史任务在“研究档案 → AI 调研”中查询。
